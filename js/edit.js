...
dept=obj.dept;
if(dept.indexOf('1')!=-1)
    objectid('php').checked=true;

if(dept.indexOf('2')!=-1)
    objectid('golang').checked=true;

if(dept.indexOf('3')!=-1)
    objectid('python').checked=true;

if(dept.indexOf('4')!=-1)
    objectid('nodejs').checked=true;

if(dept.indexOf('5')!=-1)
    objectid('react').checked=true;
...
function fun() {
    ...
    if((objectid('male').checked)==false && (objectid('female').checked)==false){
    ...
    count=(objectid("php").checked==true?"1":"") + (objectid("golang").checked==true?"1":"")+(objectid("python").checked==true?"1":"") + (objectid("nodejs").checked==true?"1":"") +(objectid("react").checked==true?"1":"");
    if(count<=2){
        dst('req12','Please select atleast 2 domain of interest',objectid("golang"));
        return false;
    }else{
        depts=(objectid("php").checked==true?"1":"") + (objectid("golang").checked==true?"2":"")+(objectid("python").checked==true?"3":"") + (objectid("nodejs").checked==true?"4":"") +(objectid("react").checked==true?"5":"");
        store("dept",depts);
    }
        ...
        if(/^[0-9]+$/.test(mobile_relative.value)==false){
            ...
            if(mn<1000000000 || mn >9999999999){
                if(mn<1000000000){
                    ...
                    dst('req29','Exceeded length.Enter valid 10 digit number', mobile_relative);
                    return false;
                }      
            }else{
...
let myPromise = new Promise(function(myResolve, myReject) {
  ...
  myPromise.then(
    function(value) {localStorage.setItem("img"+str.toString(),value);}
  );
